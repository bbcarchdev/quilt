...
static struct class_struct classes[] = {
	{
...
int
quilt_html_init(void)
{
	baseuri = config_geta("quilt:base", NULL);
	if(!baseuri)
	{
	...
	templatedir = config_geta("html:templatedir", DATAROOTDIR "/" PACKAGE_TARNAME "/templates/");
	if(!templatedir)
	{
		log_printf(LOG_CRIT, "failed to determine base path for templates\n");
		return -1;
	}
	...
	if(!liquify)
	{
		log_printf(LOG_CRIT, "failed to initialise templating context\n");
		return -1;
	}
	liquify_set_logger(liquify, log_vprintf);
...
int
quilt_html_type(const char *type)
{
	size_t c;

	for(c = 0; htmltypes[c]; c++)
	{
		if(!strcasecmp(type, htmltypes[c]))
		{
			return 1;
		}
	}
...
int
quilt_html_serialize(QUILTREQ *req)
{
	LIQUIFYTPL *tpl;
	jd_var *dict;
	char *buf;
...
static struct class_struct *
class_match(librdf_model *model, librdf_node *subject)
{   
	size_t c;
	librdf_world *world;
	librdf_stream *st;
	...
	while(!librdf_stream_end(st))
	{
		...
		if(librdf_node_is_resource(obj))
		{
			...
			if(uri)
			{
				for(c = 0; classes[c].uri; c++)
				{
					if(!strcmp(classes[c].uri, uri))
					{
						return &(classes[c]);
					}
				}
...
get_literal(QUILTREQ *req, librdf_model *model, librdf_node *subject, const char *predicate)
{
	...
	if(!obj)
	{
		log_printf(LOG_ERR, "failed to create new URI for <%s>\n", predicate);
		return NULL;
	}
	query = librdf_new_statement(world);
	if(!query)
	{
		log_printf(LOG_ERR, "failed to create new statement for query\n");
	...
	if(!subject)
	{
		log_printf(LOG_ERR, "failed to duplicate subject node\n");
		librdf_free_node(obj);
		librdf_free_statement(query);
		return NULL;
	}
	...
	if(!stream)
	{
		log_printf(LOG_ERR, "failed to create stream for model query\n");
		librdf_free_statement(query);
		return NULL;
	}
	while(!librdf_stream_end(stream))
	{
		...
		if(!obj)
		{
			log_printf(LOG_ERR, "failed to obtain object of statement\n");
		}
...
readfile(const char *path)
{
	...
	len = alloc = 0;
	buf = NULL;
	log_printf(LOG_DEBUG, "loading template: '%s'\n", path);
	f = fopen(path, "rb");
	if(!f)
	{
		log_printf(LOG_ERR, "%s: (failed to open) %s\n", path, strerror(errno));
		return NULL;
	}
	while(1)
	{
...
quilt_html_parse_(LIQUIFY *liquify, const char *name, void *data)
{
	...
	if(!pathname)
	{
		log_printf(LOG_CRIT, "failed to allocate pathname buffer\n");
		return NULL;
	}
	if(name[0] == '/')
	{
